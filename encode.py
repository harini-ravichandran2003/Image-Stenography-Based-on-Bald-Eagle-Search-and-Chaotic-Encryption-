...
def LSBEncode(coverimage, secretimage, searchimage):

    # Import an image from directory: 
    cover_image = Image.open(coverimage) 
    secret_image = Image.open(secretimage)
    search_image = Image.open(searchimage)
    ...
    # Embedding search image (bald eagle)
    se_width, se_height = search_image.size
    for i in range(width - se_width):
        for j in range(height - se_height):
            found = True
            for k in range(se_width):
                for l in range(se_height):
                    cover_pixel = output_image.getpixel((i + k, j + l))
                    search_pixel = search_image.getpixel((k, l))
                    if cover_pixel != search_pixel:
                        found = False
                        break
                if not found:
                    break
            if found:
                print(f"Bald eagle found at location ({i}, {j})")
                break
        if found:
            break
...
searchimage =  filedialog.askopenfilename(title='Select Search Image (Bald Eagle)')
LSBEncode(coverimage, secretimage, searchimage)